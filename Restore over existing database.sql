...
set @FileName = 'full_path_and_file'
...
begin
	...
	create table #backupinfo (DatabaseName varchar(128)
	...
	if charindex('.', @FileName) > 0
		set @Extention = UPPER(reverse(left(reverse(@Filename), charindex('.', reverse(@FileName)))))
	else
		set @Extention = ''
	...
	begin
		...
		IF @extention = '.SQB'
		BEGIN
			SET @Statement = N'-SQL "RESTORE SQBHEADERONLY FROM DISK = ' + char(39) + @FileName + char(39) + ' WITH SINGLERESULTSET"'
			--EXEC master..sqlbackup @Statement
			insert into #header (DatabaseName) EXEC master..sqlbackup @Statement
			-- remove unnecessary rows (only keep row with databasename)
			delete from #header where DatabaseName not like 'Database name%'
		END
		ELSE
		BEGIN
		...
		begin
			...
			IF @extention = '.SQB'
			BEGIN
				SET @Statement = N'-SQL "RESTORE FILELISTONLY FROM DISK = ' + char(39) + @FileName + char(39) + ' "'  
				--EXEC master..sqlbackup @Statement
			...
			select
				-- replace commands required to extract databasename from Redgate backups
				replace(replace(DatabaseName, 'Database name     : ', ''), 'Database name       : ', '') as DatabaseName
				, LogicalName
				, PhysicalName
				, Type
			...
			BEGIN
				...
				select @RestoreCommand = @RestoreCommand
										+ CASE WHEN @extention = '.SQB'
											THEN 'EXECUTE master..sqlbackup ''-SQL "RESTORE DATABASE [' + @DatabaseName + '] FROM '

											ELSE 'RESTORE DATABASE [' + @DatabaseName + '] FROM '
										END
				...
				where
					coalesce(db_id(@DatabaseName), database_id, 0) = coalesce(database_id, 0)
			...
			END
			IF @Extention = '.SQB'
				-- Redgate command: double all single quotes, remove the first doubled quote and add a final single quote.
				SELECT @RestoreCommand = REPLACE(
												REPLACE(@RestoreCommand, CHAR(39), char(39) + char(39))
												, 'sqlbackup ''', 'sqlbackup '
												)
											+ CHAR(39)
