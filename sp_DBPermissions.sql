IF OBJECT_ID('tempdb..#sp_DBPermissions') IS NULL
    EXEC sp_executesql N'CREATE PROCEDURE #sp_DBPermissions AS SELECT NULL as Dummy;'
GO
/*********************************************************************************************
...
ALTER PROCEDURE #sp_DBPermissions 
(
	@DBName sysname = NULL, 
...
IF @DBName COLLATE DATABASE_DEFAULT IS NULL OR @DBName COLLATE DATABASE_DEFAULT = 'All'
    BEGIN
        SET @use = ''
        IF @DBName COLLATE DATABASE_DEFAULT IS NULL
            SET @DBName = DB_NAME()
...
IF @Print = 1 AND @DBName COLLATE DATABASE_DEFAULT = 'All'
    BEGIN
...
ELSE
	-- don't display default database roles
	SET @sql = @sql + NCHAR(13) +
	N'	AND DBPrincipals.name NOT IN (''dbo'', ''guest'', ''sys'', ''db_accessadmin'', ''db_backupoperator''
									, ''db_datareader'', ''db_datawriter'', ''db_dlladmin'', ''db_denydatareader''
...
ELSE
	SET @sql = @sql + NCHAR(13) + N'	AND Users.name <> ''dbo'''
